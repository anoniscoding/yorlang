...
describe("waNinu Test Suite", () => {
    test("It should return the boolean if the parent string contains the substring", ()=> {
        ...
        expect(waNinu([oruko, "pupo"])).toBe("ooto");
    });

    test("It should fail because helper function waNinu takes only 2 arguments", () => {
        expect(()=> waNinu(["yoruba da pupo", "yoruba", "pupo"])).toThrow();
    });
